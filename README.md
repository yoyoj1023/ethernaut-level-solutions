...
23. **23-dex2**：利用 DEX 缺乏代幣白名單驗證的漏洞，創建惡意代幣並操控價格計算公式，提取流動性池中的所有代幣。
24. **24-puzzle-wallet**：利用代理合約的儲存位置衝突（Storage Collision）和 multicall 函數的邏輯缺陷，通過操控內部餘額記錄和實際合約餘額的差異來奪取管理員權限。
25. **25-motorbike**：利用 UUPS 代理合約的邏輯合約未初始化漏洞，直接初始化邏輯合約並升級為惡意合約，嘗試透過 `selfdestruct` 破壞邏輯合約（注意：EIP-6780 在 Cancun 硬分叉後限制了 `selfdestruct` 的效果）。
